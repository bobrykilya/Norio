...
class DeviceService {
	...
	static async deviceIdHandlingAndUpdating({
												 lsDeviceId,
												 fingerprint,
												 userId,
												 queryTime,
												 deviceIP,
											 }: IDeviceIdHandlingAndUpdating) {
	...
	static async createNewDeviceWithHandling({
												 interCode,
												 fingerprint,
												 userId,
												 queryTime,
	...
	static async checkDeviceForBlock({ fingerprint, queryTime, deviceId, deviceIP }: ICheckDeviceForBlock) {
		...
		const blockedDeviceInfo = await BlockRepository.getBlockedDeviceInfo({
			deviceId,
	...
	static async blockDevice({ interCode, userId, deviceId, logTime, fingerprint, deviceIP }: IBlockDevice) {
		...
		await BlockRepository.createBlock({
			...
			userId,
			lockTime: logTime,
			unlockTime: lockDuration ? getEndTime({
				startTimeInSec: logTime,
