...
export const sendToClient = <IPayload = any>({ room, event, payload }: {
	room: any;
	event: any; //! Change types
	payload: IPayload; //! Change types?
}) => {
	console.log('Leaved from room: ' + room.userId + room.deviceId)
	io
		.to(room.userId.toString() + room.deviceId.toString())
		.emit(event, payload)
}
